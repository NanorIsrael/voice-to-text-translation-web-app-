...
const QuickViewModal = () => {
  ...
  const resultedProduct = useAppSelector((state) => state.quickViewReducer.value);
  const product = {
    ...resultedProduct,
    title: resultedProduct.name,
    discountedPrice: Number(resultedProduct.price),
    imgs: {
      thumbnails: resultedProduct.product_images?.map(item => item.image),
      previews: resultedProduct.product_images?.map(item => item.image)
    }
  }
